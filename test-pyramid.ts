...
async function run() {
  ...
  await replaceInFile('./test-pyramid.template', './test-pyramid.html', [
    ...
    {
      toBeReplaced: '@TOTAL_FONC_DURATION@',
      by: NB_TOTAL_FONCTIONNEL_DURATION,
    },
    {
      toBeReplaced: '@TOTAL_FONC_PERCENT@',
      by: NB_TOTAL_FONCTIONNEL_PERCENT.toFixed(2),
    },
    { toBeReplaced: '@TOTAL_INTEG@', by: NB_TOTAL_INTEGRATION },
    {
      toBeReplaced: '@TOTAL_INTEG_DURATION@',
      by: NB_TOTAL_INTEGRATION_DURATION,
    },
    {
      toBeReplaced: '@TOTAL_INTEG_PERCENT@',
      by: NB_TOTAL_INTEGRATION_PERCENT.toFixed(2),
    },
    { toBeReplaced: '@TOTAL_UNIT@', by: NB_TOTAL_UNITAIRE },
    { toBeReplaced: '@TOTAL_UNIT_DURATION@', by: NB_TOTAL_UNITAIRE_DURATION },
    {
      toBeReplaced: '@TOTAL_UNIT_PERCENT@',
      by: NB_TOTAL_UNITAIRE_PERCENT.toFixed(2),
    },
    { toBeReplaced: '@TOTAL_AUTRE@', by: NB_TOTAL_AUTRE },
    { toBeReplaced: '@TOTAL_AUTRE_DURATION@', by: NB_TOTAL_AUTRE_DURATION },
    {
      toBeReplaced: '@TOTAL_AUTRE_PERCENT@',
      by: NB_TOTAL_AUTRE_PERCENT.toFixed(2),
    },
    {
      toBeReplaced: '@TOTAL_INSTUMENTATION_DURATION@',
      by: (TOTAL_INSTUMENTATION_DURATION / 1000).toFixed(1),
    },
  ])
}
...
async function readJsonFile(file: string): Promise<JsonReport> {
  try {
    ...
    throw new Error(
      'Error : You must run yarn test:ci before running yarn test:pyramid'
    )
  }
}
  ...
  try {
    ...
    replacements.forEach((replacement) => {
      contents = contents.replace(
        new RegExp(replacement.toBeReplaced, 'gi'),
        replacement.by.toString()
      )
    })
